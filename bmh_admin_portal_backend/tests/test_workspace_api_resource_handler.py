...
def test_workspaces_post(dynamodb_table):
    ...
    with patch("lambdas.workspaces_api_resource.workspaces_api_resource_handler.EmailHelper") as mock_email_client:
        with patch.object(workspaces_api_resource_handler, "_get_dynamodb_table_name") as mock_get_table_name:
            mock_get_table_name.return_value = "testTable"
            #Post Function returns success status
            assert workspaces_api_resource_handler._workspaces_post(email = test_email_1, body=json)['statusCode'] == 200
            ...
            assert len(retval) == 1 and retval[0]['workspace_type'] == 'STRIDES Credits' and retval[0]['request_status'] == 'pending'

            # Test grant type workspace request
            json = {'workspace_type': 'STRIDES Grant'}
            assert workspaces_api_resource_handler._workspaces_post(email = test_email_2, body=json)['statusCode'] == 200
            ...
            retval = response.get('Items',[])
            assert len(retval) == 1 and retval[0]['workspace_type'] == 'STRIDES Grant' and retval[0]['request_status'] == 'pending'
...
def test_workspace_provision(dynamodb_table):
    ...
    #Success Response#
    with mock.patch.object(workspaces_api_resource_handler , "_get_dynamodb_table_name") as mock_get_table_name:
        with mock.patch.object(workspaces_api_resource_handler , "_get_dynamodb_index_name") as mock_get_index_name:
            with mock.patch.object(workspaces_api_resource_handler, "_start_sfn_workflow") as mock_start_workflow:
                with mock.patch.object(workspaces_api_resource_handler , "_get_param") as mock_get_param:
                    with mock_apigateway():
                        with mock_sns():
                            ...
                            # Verify a success response is returned from the function
                            resp = workspaces_api_resource_handler._workspace_provision(body, path_params)
...
def test_workspace_provision_failures():
    ...
    with mock.patch.object(workspaces_api_resource_handler , "_get_dynamodb_table_name") as mock_get_table_name:
        with mock.patch.object(workspaces_api_resource_handler , "_get_dynamodb_index_name") as mock_get_index_name:
            ...
            with mock.patch.object(workspaces_api_resource_handler , "_get_workspace_request_status_and_email") as mock_get_request_status:
            ...
            with mock.patch.object(workspaces_api_resource_handler , "_get_workspace_request_status_and_email") as mock_get_request_status:
                with mock.patch.object(workspaces_api_resource_handler , "_get_param") as mock_get_param:
                    with mock_apigateway():
                        with mock_sns():
                            mock_get_request_status.return_value = 'pending', 'dummy@gmail.com'
                            mock_get_param.return_value = 'testUsageId'
                            body = {'account_id' : 'testAccount'}
                            path_params = {'workspace_id' : 'testId'}
...
def test_start_sfn_workflow():
    ...
    with patch('botocore.client.BaseClient._make_api_call', new=mock_make_api_call):
        #Success Response#
        resp = workspaces_api_resource_handler._start_sfn_workflow(workspace_id, api_key, account_id)
        assert resp['statusCode'] == '200' and resp["operation_name"] == 'StartExecution'
...
def test_workspaces_get(dynamodb_table):
    ...
    with mock.patch.object(workspaces_api_resource_handler , "_get_dynamodb_table_name") as mock_get_table_name:
        mock_get_table_name.return_value = "testTable"
        assert workspaces_api_resource_handler._workspaces_get(None, "test@uchicago.com")['statusCode'] == 204
        ...
        assert resp['statusCode'] == 200 and json.loads(resp['body'])['bmh_workspace_id'] == id2
...
def test_workspaces_set_limits(dynamodb_table):
    ...
    with mock.patch.object(workspaces_api_resource_handler , "_get_dynamodb_table_name") as mock_get_table_name:
        ...
        path_params = {}
        with pytest.raises(AssertionError):
            workspaces_api_resource_handler._workspaces_set_limits(body, path_params,test_email_1)
        ...
        body = {'soft-limit':'160', 'hard-limit':'200'}
        path_params = {'workspace_id' : id1}
        resp = workspaces_api_resource_handler._workspaces_set_limits(body, path_params,test_email_1)
        assert resp['statusCode'] == 200
        ...
        retval = response.get('Items',[])
        assert len(retval) == 1 and retval[0]['soft-limit'] == 160.0 and retval[0]['hard-limit'] == 200.0
...
def test_workspaces_set_total_usage(dynamodb_table):
    ...
    with mock.patch.object(workspaces_api_resource_handler , "_get_dynamodb_table_name") as mock_get_table_name:
        with mock.patch.object(workspaces_api_resource_handler , "_get_dynamodb_index_name") as mock_get_index_name:
            ...
            path_params = {}
            with pytest.raises(AssertionError):
                workspaces_api_resource_handler._workspaces_set_total_usage(body, path_params, api_key)
            ...
            path_params = {'workspace_id' : id1}
            with pytest.raises(AssertionError):
                workspaces_api_resource_handler._workspaces_set_total_usage(body, path_params, None)
            ...
            body = {'total-usage':'200'}
            path_params = {'workspace_id' : id1}
            with patch("lambdas.workspaces_api_resource.workspaces_api_resource_handler._publish_to_sns_topic") as mock_sns:
                resp = workspaces_api_resource_handler._workspaces_set_total_usage(body, path_params, api_key)
                assert resp['statusCode'] == 200
