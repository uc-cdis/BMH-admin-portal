...
//TODO: Form is being rendered with all the fields
//TODO: Form having default values
//TODO: Updating values in the form and make sure the updating part is happening as expected
//TODO: Verify all the client side validations for the form are done correctly.
...
const getFormData = stridesGrantWrapper => {
    ...
    /*****
    Implementing some IIFE magic to fetch the value of formData
    from the scope of requestWorkspace
    *******/
    let iifeFunction = (() => {
        return (formData, _) => {
            formDataFromState = formData;
            console.log(`This is ${formDataFromState}`);
        }
    })();
...
describe('verifies form fields are being validated correctly', () => {
    ...
    it('validates NIH IoC', async () => {
        ...
        //Enter administering_nih_institute to begin with
    ...
    it('validates NIH grant number', async () => {
        ...
        //Enter nih_funded_award_number to begin with
        input = stridesGrantWrapper.find('FormControl').filter({'name': fieldName}).filterWhere((n) => n.prop('onChange'));

        //Update the nih_funded_award_number with a non empty value
        let custom_input_change_event = {
            target :{
                name : fieldName,
                value : updatedFormData[fieldName],
